...
Workflow:
- Never run whole-repo `bun typecheck`. It requires so much resources that makes the system unresponsive. Scope typecheck to one package.
- Never run typecheck in any package, on an aarch64 linux (low-end) device.
- If you need to run `git push`, add `--no-verify`. The push hooks are too slow to run.
